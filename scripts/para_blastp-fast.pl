...
if ($threads == 1) {
    print `$program -query $query -db $db -out $out -outfmt $outfmt -evalue $evalue -num_threads $threads -max_target_seqs $max_hits -task blastp-fast`;
}
else {
	...
	my $blast_exe = "$program -query $tmp_dir/split-$i.fsa -db $db -num_threads $splitby -out $tmp_dir/result_splits/split.$i.txt -max_target_seqs $max_hits -evalue $evalue -outfmt $outfmt -task blastp-fast";
